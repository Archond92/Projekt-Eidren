...
{
	public sealed class MidpolyGraniteShellProductionTests
	{
		private const string PrefabPath = "Assets/_Game/Prefabs/Actors/3D/GraniteShell_3D.prefab";
		private const string EnemyPath = "Assets/_Game/Prefabs/Enemies/TierTwo/GraniteShell.prefab";
		private const string GlbPath = "Assets/_Game/Art/MidPoly/GoldenMasters/GraniteShell/Runtime/CRE_GraniteShell_Mid_Production.glb";
		...
		[Test]
		public void GraniteShellPrefab_PreservesProductionContract()
		{
			GameObject prefab = AssetDatabase.LoadAssetAtPath<GameObject>(PrefabPath);
			Assert.That(prefab, Is.Not.Null);
			Assert.That(AssetDatabase.AssetPathToGUID(PrefabPath), Is.EqualTo("f4b7c3102920e4749a05285dd981a61c"));
			Assert.That(AssetDatabase.GetLabels(prefab), Does.Contain("GraniteShellGoldenMaster"));
			Assert.That(prefab.GetComponent<CreatureMeshPresentation>(), Is.Not.Null);
			Assert.That(prefab.GetComponent<CreatureMeshPresentation>().WorldHeight, Is.EqualTo(2.40f).Within(0.001f));
			...
			Assert.That(Triangles(RenderersFor(renderers, "LOD0")), Is.EqualTo(16868));
			Assert.That(Triangles(RenderersFor(renderers, "LOD1")), Is.EqualTo(9108));
			Assert.That(Triangles(RenderersFor(renderers, "LOD2")), Is.EqualTo(3681));
			...
			Assert.That(AssetDatabase.AssetPathToGUID(EnemyPath), Is.EqualTo("8d434a6fffb80534889f6fb14f2f8ed1"));
			CapsuleCollider collider = enemy.GetComponent<CapsuleCollider>();
			Assert.That(collider.radius, Is.EqualTo(0.676f).Within(0.001f));
			Assert.That(collider.height, Is.EqualTo(2.73f).Within(0.001f));
			Assert.That(enemy.GetComponentInChildren<CreatureMeshPresentation>(true), Is.Not.Null);
		}

		[Test]
		public void GraniteShellSkin_AllLodsHaveNormalizedWeightsAndExactRig()
		{
			GameObject prefab = AssetDatabase.LoadAssetAtPath<GameObject>(PrefabPath);
		...
		[Test]
		public void GraniteShellClips_SampleFiniteAndKeepReadableFootprint()
		{
			GameObject prefab = AssetDatabase.LoadAssetAtPath<GameObject>(PrefabPath);
			...
			try
			{
				...
				foreach (AnimationClip clip in clips.Values)
				{
					...
					for (int sample = 0; sample <= 6; sample++)
					{
						...
						Assert.That(bounds.size.x, Is.LessThan(3.10f), clip.name);
						Assert.That(bounds.size.y, Is.LessThan(3.20f), clip.name);
						Assert.That(bounds.size.z, Is.LessThan(4.00f), clip.name);
						Assert.That(bounds.min.y, Is.GreaterThan(-0.60f), clip.name);
